package edu.kh.exception.modle.service;
...
public class ExceptionService {
	...
	public void ex1() {
		...
		// 27번에서 오류가 생길 수 있을거 같음 그러면?
		try {
			System.out.print("결과 : " + input1 / input2);
		} catch(ArithmeticException e) {  // e : 변수명
			// try에서 던저진 예외를 catch문 매개변수로 잡음
		...
		// 발생하는 예외 중 일부 예외는 try - catch 구문을 사용하지 않아도 예외상황 방지 가능
		// (일부 예외 == 대부분의 UnchechedException)
	...
	public void ex3() {
		...
		// *NullPointerException : 참조하지 않는 참조변수를 이용해서 코드를 소행할 때 발생
	...
	public void ex4() {
		...
		try {
			...
			System.out.println(e); // e.toStirng();
			// ==> java.lang.ArithmeticException: / by zero
			
			e.printStackTrace(); // 예외가 발생하기 까지의 모든 메소드 흐름을 출력
			// 어디서 어떤 에러가 발새했는지 정확히 파악하는 용도
	...
	public void methodC() throws IOException {
		...
		// 2) throws로 호출한 메서드로 예외를 위윔하여 처리
	}
